namespace SimpleExecTests
{
    ...
    public class CancellingCommands
    {
        ...
        public void RunningACommandAsync(CancellationTokenSource cancellationTokenSource, Task command, Exception exception)
        {
            ...
            "Then a OperationCanceledException is Thrown"
        ...
        [Scenario]
        public void RunningACommand(CancellationTokenSource cancellationTokenSource, Action command, Exception exception)
        {
            "Given a cancellation token source"
                .x(c => cancellationTokenSource = new CancellationTokenSource().Using(c));
            ...
            "When I start running a long-running command using the cancellation token"
                .x(() =>
                {
                    command = () => Command.Run("dotnet", $"exec {Tester.Path} sleep", cancellationToken: cancellationTokenSource.Token);
                });

            "And I cancel the cancellation token source"
                .x(() => cancellationTokenSource.Cancel());

            "And I wait for the command"
                .x(() => exception = Record.Exception( () => command()));
        ...
        [Scenario]
        public void ReadingACommand(CancellationTokenSource cancellationTokenSource, Action command, Exception exception)
        {
            "Given a cancellation token source"
                .x(c => cancellationTokenSource = new CancellationTokenSource().Using(c));
            ...
            "When I start reading a long-running command using the cancellation token"
                .x(() =>
                {
                    command = () => Command.Read("dotnet", $"exec {Tester.Path} sleep", cancellationToken: cancellationTokenSource.Token);
                });
